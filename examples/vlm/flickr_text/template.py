...
def load_inference_tensors(tensors_dir: Path, split: str) -> List[Dict[str, torch.Tensor]]:
    ...
    Expects files produced by openlvlm-mia/main.py:
        - images.safetensors      (not used here)
        - input_ids.safetensors
        - logits.safetensors
        - labels.safetensors

    Returns a dict mapping sample_id -> {logits, input_ids, label}.
    """
    logits_path = tensors_dir / "logits.safetensors"
    input_ids_path = tensors_dir / "input_ids.safetensors"
    labels_path = tensors_dir / "labels.safetensors"

    if not logits_path.exists() or not input_ids_path.exists() or not labels_path.exists():
        raise FileNotFoundError(
            f"Expected logits/input_ids/labels safetensors in {tensors_dir}, "
            "but one or more files are missing."
        )

    logits_tensors = load_safetensors(str(logits_path))
    input_ids_tensors = load_safetensors(str(input_ids_path))
    ...
    for key in sorted(logits_tensors.keys()):
        if not key.startswith("logits_"):
            continue
        sample_id = key.split("_", 1)[1]
        logits = logits_tensors[key]
        input_key = f"input_ids_{sample_id}"
        label_key = f"label_{sample_id}"
        if input_key not in input_ids_tensors or label_key not in labels_tensors:
            logging.warning(f"Missing input_ids/label for sample {sample_id}, skipping.")
            continue
        sample_dict[sample_id] = {
            "logits": logits.clone().detach().to(dtype=torch.float32, device="cpu"),
            "input_ids": input_ids_tensors[input_key].clone().detach().to(dtype=torch.long, device="cpu"),
...
def main() -> None:
    ...
    tensors_dir = Path("/lustre/orion/lrn087/proj-shared/toan/vr-mia/src/mia_agents_dual/examples/vlm/openlvlm-mia/outputs/llava_flickr/inference_tensors")
    ...
    for data in tqdm(samples, desc="Computing MIA signals"):
        ...
        signal = compute_mia_signal(logits, input_ids)
